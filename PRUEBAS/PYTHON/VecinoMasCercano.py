...
matriz5 = np.array([[0, 26, 17, 15, 27], 
                    [26, 0, 15, 12, 4], 
                    [17, 15, 0, 2, 18],
                    [15, 12, 2, 0, 17], 
                    [27, 4, 18, 17, 0]])
...
def calcularRuta(matriz, indice, cont):
    ...
    global ciudadesAux 
    ciudadesAux.append(indice)#añadimos al array ciudades la ciudad que este en el indice que recibimos por parametro

    matriz[:, indice] = 0 #convierte la columna que corresponde al indice del array que se le ha pasado como argumento a la funcion en todo 0

    cont = cont + 1 #incrementamos el contador en 1

    if cont < len(matriz): #si el contador llega al numero de ciudades que hay en la matriz sale de la funcion

        ciudadMasCercana = np.min(matriz[indice][np.nonzero(matriz[indice])]) #obtiene el valor minimo de un array obviando el 0
        indicesConValorMinimo = np.where(matriz[indice] == ciudadMasCercana) #devuelve una tupla, el primer valor es un array con los indices donde ha encontrado ese valor min (puede darse el caso de que este repetido y salga mas de un valor) y el segundo valor es el tipo de dato  
        
        if len(indicesConValorMinimo[0]) > 1:#comprobar si tiene mas de uno, si lo tiene habria que recorrer este array comprobando cada indice en la matriz para ver que camino es el mas eficiente y escogerlo
            print ("AAAA")
        
        calcularRuta(matriz, indicesConValorMinimo[0][0], cont) #llamamos a la funcion pasandole la matriz, el indice con el que tiene que trabajar y un contador de ciclos
