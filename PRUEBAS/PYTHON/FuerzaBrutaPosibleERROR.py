...
def distancia(ciudade,mejorDistancia):
    ciu = []
    ciu = ciudade[:]
    distancia=0
    mejorRuta=[]
    ciu.append(ciudade[0])
    for i in range(len(ciu) - 1):
        distancia = distancia+matriz[ciu[i]][ciu[i+1]]
    print(distancia)
    if distancia < mejorDistancia:
        mejorDistancia = distancia
        mejorRuta = ciu
        print (f'Ruta: {mejorRuta}')
        print (f'Distancia: {mejorDistancia}')
...
while contador1 < mitadFactorial -1:
    ...
    for i in range(len(ciudades) - 1): #ESTO OK
        mejorDistancia=distancia(ciudades,mejorDistancia)
